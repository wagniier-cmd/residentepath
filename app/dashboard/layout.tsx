...
const NAV_HREFS = [
  ...
  { key: 'questions' as const, href: '/dashboard/questions' },
  { key: 'flashcards' as const, href: '/dashboard/flashcards' },
  { key: 'simulado' as const, href: '/dashboard/simulado' },
  { key: 'ranking' as const, href: '/dashboard/ranking' },
  { key: 'perfil' as const, href: '/dashboard/perfil' },
]
...
const NAV_ICONS = {
  ...
  questions: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>,
  flashcards: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>,
  simulado: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  ranking: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></svg>,
  perfil: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>,
...
function DashboardLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  const navItems = NAV_HREFS.map(({ key, href }) => ({
    href,
    label: t.nav[key],
    icon: NAV_ICONS[key],
  }))
  ...
  const visibleNav = isAdmin
    ? [...navItems, { href: '/admin', label: t.nav.admin, icon: NAV_ICONS.admin }]
    : navItems
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-primary-700 text-white fixed inset-y-0 left-0 z-30">
        ...
        <div className="px-4 pb-2 border-t border-white/10 pt-4">
          <p className="text-xs text-white/40 px-4 mb-2 uppercase tracking-wide">Interface</p>
            ...
            {LANGUAGE_OPTIONS.map(opt => (
              <button
                ...
                className={`px-1.5 py-1 rounded text-sm transition-all ${
                  language === opt.value ? 'bg-white/20' : 'text-white/50 hover:text-white'
                }`}
              >
                {opt.flag}
              </button>
